...
import net.essence.client.particles.EntityDepthsPotalFX;
...
public class BlockDepthsPortal extends BlockBreakable {
	...
	public BlockDepthsPortal(String name) {
		super(SlayerAPI.PREFIX + "depthsPortal", Material.portal, false);
	...
	public boolean makePortal(World w, int x, int y, int z) {
		BlockDepthsPortal.Size size = new BlockDepthsPortal.Size(w, x, y, z, 1);
		BlockDepthsPortal.Size size1 = new BlockDepthsPortal.Size(w, x, y, z, 2);
	...
	public void randomDisplayTick(World world, int xPos, int yPos, int zPos, Random rand) {
        for (int l = 0; l < 5; ++l) {
            ...
            EntityDepthsPotalFX var20 = new EntityDepthsPotalFX(world, d0, d1, d2, d3, d4, d5);
	...
	public static class Size {
		...
		protected int setDirection(int x, int y, int z, int dir) {
			...
			for(i1 = 0; i1 < 22; ++i1) {
				...
				if(block1 != EssenceBlocks.depthsPortalFrame) {
					break;
				}
			}
		...
		protected int idk() {
			...
			label56:
				...
				for(this.direc = 0; this.direc < 21; ++this.direc) {
					...
					for(j = 0; j < this.sides; ++j) {
						k = this.coords.posX + j * Direction.offsetX[BlockDepthsPortal.sides[this.x][1]];
						l = this.coords.posZ + j * Direction.offsetZ[BlockDepthsPortal.sides[this.x][1]];
			...
			for(i = 0; i < this.sides; ++i) {
				j = this.coords.posX + i * Direction.offsetX[BlockDepthsPortal.sides[this.x][1]];
				k = this.coords.posY + this.direc;
				l = this.coords.posZ + i * Direction.offsetZ[BlockDepthsPortal.sides[this.x][1]];
		...
		public void checkDirection() {
			for(int i = 0; i < this.sides; ++i) {
				...
				for(int l = 0; l < this.direc; ++l) {
					...
					this.w.setBlock(j, i1, k, EssenceBlocks.depthsPortal, this.x, 2);
				}
			}
		}
	}
