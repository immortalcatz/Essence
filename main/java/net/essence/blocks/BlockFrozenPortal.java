...
public class BlockFrozenPortal extends BlockBreakable {
	...
	public BlockFrozenPortal(String name) {
		super(SlayerAPI.PREFIX + "frozenPortal", Material.portal, false);
	...
	public boolean makePortal(World w, int x, int y, int z) {
		BlockFrozenPortal.Size size = new BlockFrozenPortal.Size(w, x, y, z, 1);
		BlockFrozenPortal.Size size1 = new BlockFrozenPortal.Size(w, x, y, z, 2);
	...
	public void randomDisplayTick(World world, int xPos, int yPos, int zPos, Random rand) {
        for (int l = 0; l < 5; ++l) {
            ...
            //EntityfrozenPotalFX var20 = new EntityfrozenPotalFX(world, d0, d1, d2, d3, d4, d5);
	...
	public static class Size {
		...
		protected int idk() {
			...
			label56:
				...
				for(this.direc = 0; this.direc < 21; ++this.direc) {
					...
					for(j = 0; j < this.sides; ++j) {
						k = this.coords.posX + j * Direction.offsetX[BlockFrozenPortal.sides[this.x][1]];
						l = this.coords.posZ + j * Direction.offsetZ[BlockFrozenPortal.sides[this.x][1]];
			...
			for(i = 0; i < this.sides; ++i) {
				j = this.coords.posX + i * Direction.offsetX[BlockFrozenPortal.sides[this.x][1]];
				k = this.coords.posY + this.direc;
				l = this.coords.posZ + i * Direction.offsetZ[BlockFrozenPortal.sides[this.x][1]];
		...
		public void checkDirection() {
			for(int i = 0; i < this.sides; ++i) {
				...
				for(int l = 0; l < this.direc; ++l) {
					...
					this.w.setBlock(j, i1, k, EssenceBlocks.frozenPortal, this.x, 2);
				}
			}
		}
	}
