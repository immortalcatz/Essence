...
public class BlockBoilPortal extends BlockBreakable {
	...
	public static final PropertyEnum pe = PropertyEnum.create("axis", EnumFacing.Axis.class, new EnumFacing.Axis[] {EnumFacing.Axis.X, EnumFacing.Axis.Z});
	...
	public void setBlockBoundsBasedOnState(IBlockAccess w, BlockPos pos) {
		EnumFacing.Axis axis = (EnumFacing.Axis)w.getBlockState(pos).getValue(pe);
		float f = 0.125F;
		float f1 = 0.125F;
	...
	public void onNeighborBlockChange(World worldIn, BlockPos pos, IBlockState state, Block neighborBlock) {
		EnumFacing.Axis axis = (EnumFacing.Axis)state.getValue(pe);
		BlockBoilPortal.Size size;

		if (axis == EnumFacing.Axis.X) {
			size = new BlockBoilPortal.Size(worldIn, pos, EnumFacing.Axis.X);

			if (!size.getSize() || size.field_150864_e < size.field_150868_h * size.field_150862_g) {
	...
	public boolean shouldSideBeRendered(IBlockAccess worldIn, BlockPos pos, EnumFacing side) {
		...
		if (worldIn.getBlockState(pos).getBlock() == this) {
			axis = (EnumFacing.Axis)iblockstate.getValue(pe);
	...
	public IBlockState getStateFromMeta(int meta) {
		return this.getDefaultState().withProperty(pe, (meta & 3) == 2 ? EnumFacing.Axis.Z : EnumFacing.Axis.X);
	}
	...
	public int getMetaFromState(IBlockState state) {
		return getMeta((EnumFacing.Axis)state.getValue(pe));
	}

	public static int getMeta(EnumFacing.Axis a) {
	...
	protected BlockState createBlockState() {
		return new BlockState(this, new IProperty[] {pe});
	}
	...
	public static class Size
	{
		...
		public Size(World worldIn, BlockPos p_i45694_2_, EnumFacing.Axis p_i45694_3_)
		{
			...
			for (BlockPos blockpos1 = p_i45694_2_; p_i45694_2_.getY() > blockpos1.getY() - 21 && p_i45694_2_.getY() > 0 && this.getMat(worldIn.getBlockState(p_i45694_2_.offsetDown()).getBlock()); p_i45694_2_ = p_i45694_2_.offsetDown())
			{
				;
			}
		...
		protected int func_180120_a(BlockPos p_180120_1_, EnumFacing p_180120_2_)
		{
			...
			for (i = 0; i < 22; ++i)
			{
				...
				if (!this.getMat(this.field_150867_a.getBlockState(blockpos1).getBlock()) || this.field_150867_a.getBlockState(blockpos1.offsetDown()).getBlock() != EssenceBlocks.boilPortalFrame)
				{
					break;
				}
			}
		...
		protected int func_150858_a() {
			...
			for (i = 0; i < this.field_150868_h; ++i) {
				if (this.field_150867_a.getBlockState(this.field_150861_f.offset(this.field_150866_c, i).offsetUp(this.field_150862_g)).getBlock() != EssenceBlocks.boilPortalFrame) {
		...
		public void checkDir() {
			for (int i = 0; i < this.field_150868_h; ++i) {
				...
				for (int j = 0; j < this.field_150862_g; ++j) {
					this.field_150867_a.setBlockState(blockpos.offsetUp(j), EssenceBlocks.boilPortal.getDefaultState().withProperty(BlockPortal.field_176550_a, this.field_150865_b), 2);
				}
			}
		}
	}
