...
public class BlockEucaPumpkin extends BlockMod {
	...
	protected BlockPattern getFourfaPattern() {
		if(this.fourfaPattern == null)
			this.fourfaPattern = FactoryBlockPattern.start().aisle(new String[] {"^^", "OO", "##"}).where('O', BlockWorldState.hasState(BlockStateHelper.forBlock(EssenceBlocks.celestiumOre))).where('^', BlockWorldState.hasState(BlockStateHelper.forBlock(EssenceBlocks.eucaPumpkin))).where('#', BlockWorldState.hasState(BlockStateHelper.forBlock(EssenceBlocks.eucaStone))).build();
