...
public class ItemDemonicEye extends ItemMod {
	...
	public boolean onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ) {
		...
		if (playerIn.canPlayerEdit(pos.offset(side), side, stack) && iblockstate.getBlock() == EssenceBlocks.corbaPortalFrame && !((Boolean)iblockstate.getValue(BlockCorbaPortalFrame.EYE)).booleanValue()) {
			if (worldIn.isRemote) {
				return true;
			} else {
				worldIn.setBlockState(pos, iblockstate.withProperty(BlockCorbaPortalFrame.EYE, Boolean.valueOf(true)), 2);
				worldIn.updateComparatorOutputLevel(pos, EssenceBlocks.corbaPortalFrame);
				--stack.stackSize;
				...
				for (int k = -2; k <= 2; ++k) {
					...
					if (iblockstate1.getBlock() == EssenceBlocks.corbaPortalFrame) {
				...
				if (flag && j == l + 2) {
					...
					for (i1 = l; i1 <= j; ++i1) {
						...
						if (iblockstate3.getBlock() != EssenceBlocks.corbaPortalFrame || !((Boolean)iblockstate3.getValue(BlockCorbaPortalFrame.EYE)).booleanValue()) {
							flag = false;
							break;
						}
					}
					...
					for (i1 = l - 1; i1 <= j + 1; i1 += 4) {
						...
						for (j1 = 1; j1 <= 3; ++j1) {
							...
							if (iblockstate2.getBlock() != EssenceBlocks.corbaPortalFrame || !((Boolean)iblockstate2.getValue(BlockCorbaPortalFrame.EYE)).booleanValue()) {
								flag = false;
								break;
							}
						}
					...
					if (flag) {
						for (i1 = l; i1 <= j; ++i1) {
							...
							for (j1 = 1; j1 <= 3; ++j1) {
								blockpos4 = blockpos1.offset(enumfacing1, j1);
								worldIn.setBlockState(blockpos4, EssenceBlocks.corbaPortal.getDefaultState(), 2);
							}
						}
					}
				}
