...
public class WorldGenBoilPortal extends WorldGenerator {
	...
	public boolean generate(World w, Random r, BlockPos pos) {
		...
		for(i = 0; i < 100; i++) {
			for(int x1 = 0; x1 < x - 6; x1++) {
				for(int z1 = 0; z1 < z + 26; z1++) {
					if(w.getBlockState(new BlockPos(x + x1, i, z + z1)) == Blocks.netherrack.getDefaultState()) {
						foundGround = true;
						break;
					}
				}
		...
		for(i = 0; i < 7; i++){
			w.setBlockState(new BlockPos(x + 1, y - 4 + i, z + 5 - i), Blocks.nether_brick_stairs.getStateFromMeta(3), 2);
			w.setBlockState(new BlockPos(x + 2, y - 4 + i, z + 5 - i), Blocks.nether_brick_stairs.getStateFromMeta(3), 2);
		}*/
