...
public class WorldGenDepthsLights extends WorldGenerator {
	...
	public boolean generate(World w, Random r, BlockPos pos) {
		...
		else if (w.getBlockState(pos.up()).getBlock() != EssenceBlocks.depthsStone) {
			return false;
		} else {
			w.setBlockState(pos, EssenceBlocks.depthsLights.getDefaultState(), 2);
			for(int i = 0; i < 1500; i++) {
				...
				if(w.getBlockState(blockpos1).getBlock().getMaterial() == Material.air) {
					...
					for(int l = 0; l < k; l++) {
						...
						if (w.getBlockState(blockpos1.offset(enumfacing)).getBlock() == EssenceBlocks.depthsLights) j++;
						if(j > 1) break;
					}
					if(j == 1) w.setBlockState(blockpos1, EssenceBlocks.depthsLights.getDefaultState(), 2);
				}
			}
			return true;
		}
