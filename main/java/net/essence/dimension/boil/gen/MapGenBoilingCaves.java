...
public class MapGenBoilingCaves extends MapGenBase {
	...
	protected void gen2(long l, int i, int j, Block[] b, double d, double d1, double d2) {
        this.gen(l, i, j, b, d, d1, d2, 1.0F + this.rand.nextFloat() * 6.0F, 0.0F, 0.0F, -1, -1, 0.5D);
    }

    protected void gen(long l, int i, int j, Block[] b, double d, double d1, double d2, float f, float f1, float f2, int k, int l4, double d3) {
        ...
        for (boolean flag = random.nextInt(6) == 0; k < l4; ++k) {
            ...
            if (!flag2 && k == k1 && f > 1.0F && l4 > 0) {
                this.gen(random.nextLong(), i, j, b, d, d1, d2, random.nextFloat() * 0.5F + 0.5F, f1 - ((float)Math.PI / 2F), f2 / 3.0F, k, l4, 1.0D);
                this.gen(random.nextLong(), i, j, b, d, d1, d2, random.nextFloat() * 0.5F + 0.5F, f1 + ((float)Math.PI / 2F), f2 / 3.0F, k, l4, 1.0D);
                return;
            }
    ...
    protected void func_151538_a(World w, int x, int y, int z, int i, Block[] ba) {
        ...
        for (int j1 = 0; j1 < i1; ++j1) {
            ...
            if (this.rand.nextInt(4) == 0) {
                this.gen2(this.rand.nextLong(), z, i, ba, d0, d1, d2);
            ...
            for (int l1 = 0; l1 < k1; ++l1) {
                ...
                this.gen(this.rand.nextLong(), z, i, ba, d0, d1, d2, f2, f, f1, 0, 0, 1.0D);
            }
        }
    }
