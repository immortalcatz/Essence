...
public class ChunkProviderBoiling implements IChunkProvider {
					...
					for(int l2 = 0; l2 < 8; ++l2) {
						...
						for(int i3 = 0; i3 < 4; ++i3) {
							...
							for (int j3 = 0; j3 < 4; ++j3) {
								if ((d15 += d16) > 0.0D) {
									p_180518_3_.setBlockState(k * 4 + i3, k2 * 8 + l2, j1 * 4 + j3, EssenceBlocks.ashBlock.getDefaultState());
								}
								else if (k2 * 8 + l2 < 69) {
	...
	public final void generateBiomeTerrain(Random r, ChunkPrimer c, int x, int z, double s) {
		...
		IBlockState iblockstate1 = EssenceBlocks.ashBlock.getDefaultState();
		int k = -1;
