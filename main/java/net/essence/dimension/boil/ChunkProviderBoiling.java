...
public class ChunkProviderBoiling implements IChunkProvider {
	...
	public ChunkProviderBoiling(World var1, long var2){
		this.worldObj = var1;
					...
					for (int l1 = 0; l1 < 4; ++l1) {
						...
						for (int i2 = 0; i2 < 8; ++i2) {
							...
							for (int j2 = 0; j2 < 8; ++j2) {
								...
								if (d15 > 0.0D) {
									iblockstate = EssenceBlocks.ashBlock.getDefaultState();
								}
		...
		for (int i = 0; i < 16; ++i) {
			for (int j = 0; j < 16; ++j) {
				...
				for (int l = 127; l >= 0; --l) {
					...
					else if (iblockstate2.getBlock() == EssenceBlocks.ashBlock) {
						if (k == -1) {
							if (b0 <= 0) {
							...
							if (l >= 0) {
								...
								if(c.getBlockState(i, l - 1, j) == EssenceBlocks.ashBlock.getDefaultState()) c.setBlockState(i, l - 1, j, iblockstate1);
								if(c.getBlockState(i, l - 2, j) == EssenceBlocks.ashBlock.getDefaultState() && rand.nextInt(2) == 0) c.setBlockState(i, l - 2, j, iblockstate1);
							}
						}
						else if (k > 0) {
							--k;
